...
function Generate() {
  ...
  const handleGeneratePDF = async () => {
    ...
    try {
      ...
      const pdfBlob = await generatePDF({
        ...
        lineItemConfig: state.lineItemConfig,
  ...
  const isSetupComplete = () => {
    return (
      state.airtableConfig.apiKey &&
      state.airtableConfig.baseId &&
      state.airtableConfig.tableName &&
      state.googleDocUrl &&
          ...
          <div className="flex items-center space-x-3">
            ...
            <div>
              ...
              <p className="text-sm text-yellow-700 mt-1">
                Please complete the setup process before generating PDFs. You need to:
              </p>
              <ul className="text-sm text-yellow-700 mt-2 space-y-1">
                {!state.airtableConfig.apiKey && <li>• Configure Airtable connection</li>}
                {!state.googleDocUrl && <li>• Add Google Docs template URL</li>}
                {state.templateFields.length === 0 && <li>• Process your template to detect fields</li>}
                {Object.keys(state.fieldMappings).length === 0 && <li>• Map template fields to Airtable columns</li>}
              </ul>
            </div>
          </div>
        </motion.div>
              ...
              <div className="bg-gray-50 p-4 rounded-lg">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm">
                  <div>
                    ...
                    <span className="ml-2 text-gray-900">{state.templateFields.length}</span>
                  </div>
                  <div>
                    <span className="font-medium text-gray-600">Field Mappings:</span>
                    <span className="ml-2 text-gray-900">{Object.keys(state.fieldMappings).length}</span>
                  </div>
                  <div>
                    <span className="font-medium text-gray-600">Line Items:</span>
                    <span className="ml-2 text-gray-900">
                      {state.lineItemConfig.enabled ? 'Enabled' : 'Disabled'}
                    </span>
                  </div>
                  <div>
